    ...
    <main className="min-h-screen bg-background">
      ...
      <section className="security-grid border-b border-border">
        <div className="mx-auto max-w-5xl px-5 py-14 sm:px-8 sm:py-20">
          ...
          <p className="mt-5 font-mono text-xs uppercase tracking-[0.12em] text-muted-foreground">Last updated: July 30, 2026</p>
        </div>
      </section>

      <section className="mx-auto grid max-w-7xl gap-10 px-5 py-10 sm:px-8 lg:grid-cols-[13rem_minmax(0,1fr)] lg:py-14">
        <aside className="h-fit rounded-lg border border-border bg-card/55 p-4 lg:sticky lg:top-6">
          <p className="mb-3 font-mono text-[11px] uppercase tracking-[0.14em] text-primary">Trust center</p>
          <nav className="grid gap-1 text-sm">
            {legalLinks.map(([label, href]) => (
              <Link key={href} href={href} className="rounded-md px-3 py-2 text-muted-foreground transition-colors hover:bg-primary/10 hover:text-primary">
                {label}
              </Link>
            ))}
          </nav>
      ...
      <footer className="border-t border-border bg-muted/20">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-5 py-8 text-sm text-muted-foreground sm:px-8 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-2"><ShieldCheck className="size-4 text-primary" /> Tri-Proof Guard trust and legal center.</div>
