...
export function DeveloperAccess() {
  ...
  async function load() {
    ...
    if (apiResponse.ok) {
      ...
      setMonthlyLimit(body.monthlyLimit ?? 0)
    }
    if (groupResponse.ok) {
      const body = await groupResponse.json()
      setGroups(body.groups ?? [])
      setGroupLimit(body.groupLimit ?? 0)
    }
  }
