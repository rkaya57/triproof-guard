    ...
    <Card className="glass-panel premium-card border-violet-400/25">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="rounded-xl border border-violet-400/20 bg-violet-400/5 p-4 text-sm leading-6 text-slate-300">
          Use the blind second-review CSV generated by Internal Calibration after it has been completed. Only cases originally selected as <strong className="text-white">sybil</strong> or <strong className="text-white">bot</strong> may enter this adjudication layer. Reviewer independence is measured and reported, but even a fully independent second pass remains internal and claim-ineligible here.
        </div>
        ...
        {result ? (
          <div className="space-y-6">
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              <MetricComparison label="Acceptable decision accuracy" before={percent(result.original.metrics.acceptableDecisionAccuracy)} after={percent(result.adjudicated.metrics.acceptableDecisionAccuracy)} />
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              <div className="rounded-xl border border-border p-4 text-sm text-slate-300">
                ...
                <div className="mt-3 space-y-2">
                  ...
                  <p>Independent reviewer cases: <strong className="text-white">{result.provenance.independentReviewerCases}</strong></p>
                  <p>Independence gate: <strong className={result.provenance.independenceSatisfied ? "text-green-300" : "text-amber-300"}>{result.provenance.independenceSatisfied ? "SATISFIED" : "NOT SATISFIED"}</strong></p>
                </div>
                ...
                <table className="w-full text-left text-xs">
                  <thead className="sticky top-0 bg-background text-slate-400">
                    <tr>
                      ...
                      <th className="px-3 py-2">Independent</th>
                    </tr>
                  </thead>
                  <tbody>
                    {result.changes.map((item) => (
                      <tr key={item.caseId} className="border-t border-border/60 text-slate-300">
                        ...
                        <td className={item.independentReviewer ? "px-3 py-2 text-green-300" : "px-3 py-2 text-amber-300"}>{item.independentReviewer ? "yes" : "no"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
