  ...
  return (
    <div className="flex flex-col gap-6">
      <section className="dashboard-hero reveal-up rounded-2xl p-6 sm:p-8">
        ...
        <h2 className="text-gradient text-3xl font-semibold sm:text-4xl">USDC and SOL payments</h2>
        <p className="mt-3 max-w-3xl text-muted-foreground">Payments are accepted only on Solana mainnet. The server verifies each successful on-chain transfer before granting campaign-analysis credits.</p>
      </section>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        ...
        <Card className="glass-panel premium-card"><CardHeader><CardTitle className="flex items-center gap-2"><Coins className="text-primary" /> Live plan ledger</CardTitle><CardDescription>Plan values use a USDC denomination. SOL checkout uses a signed, 15-minute live equivalent.</CardDescription></CardHeader><CardContent className="grid gap-3">
          {Object.values(billingPlans).map((plan) => <div key={plan.id} className="flex items-center justify-between rounded-xl border border-border bg-background/45 p-4"><div><p className="font-medium text-white">{plan.name}</p><p className="mt-1 text-xs text-slate-400">{plan.walletCredits.toLocaleString("en-US")} wallet credits</p></div><p className="text-xl font-semibold text-primary">{plan.amountUsdc} USDC</p></div>)}
        </CardContent></Card>
      </section>
