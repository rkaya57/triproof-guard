    ...
    <div className="flex flex-col gap-7">
      <section className="glass-panel rounded-3xl p-6 sm:p-8">
        <div className="mb-5 flex flex-wrap items-center gap-3">
          ...
          <Badge variant="outline" className="border-yellow-400/30 bg-yellow-400/10 text-yellow-200">Controlled merge</Badge>
        </div>
        <h1 className="text-gradient text-4xl font-semibold sm:text-5xl">Humanity Gate Demo</h1>
        <p className="mt-4 max-w-3xl text-slate-300">
          This admin-only sandbox uses the real Humanity database tables and API routes. The camera/MediaPipe detector is still held back; this page submits derived test scores to validate the production flow safely.
        </p>
      ...
      <section className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
        <Card className="glass-panel premium-card animated-border">
          <CardHeader>
            ...
            <CardDescription className="text-slate-300">Choose campaign, chain and wallet to create a live DB session.</CardDescription>
          </CardHeader>
        ...
        <Card className="glass-panel premium-card animated-border">
          ...
          <CardContent className="grid gap-4">
            {!session ? (
              <p className="rounded-xl border border-border bg-background/45 p-4 text-sm text-slate-300">Start a session to see the challenge sequence.</p>
            ) : (
              <>
