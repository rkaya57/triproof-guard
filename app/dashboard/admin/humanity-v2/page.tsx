...
export default async function Page() {
    ...
    return (
      <Card className="glass-panel mx-auto max-w-2xl">
        <CardHeader>
          ...
          <CardDescription>Log in with a Tri-Proof admin email to inspect Humanity Liveness.</CardDescription>
        </CardHeader>
  ...
  const summaryCards: Array<{ icon: LucideIcon; label: string; value: number; detail: string }> = [
    ...
    { icon: CircleDashed, label: "Recent verifications", value: recentVerifications.length, detail: "Latest Humanity verification records" },
    { icon: WalletCards, label: "Signed proofs", value: signedCount, detail: "Cryptographically verified wallet signatures" },
    { icon: TriangleAlert, label: "Review queue", value: reviewCount, detail: "Server-chain proofs remain review-only by design" },
  ]
  ...
  return (
    <div className="grid gap-6 pb-10">
      <AdminWorkspaceHeader
        icon={Fingerprint}
        eyebrow="Humanity Liveness"
        title="Proof-of-human recovery console"
        description="Tri-Proof Humanity Liveness combines motion verification, capture integrity, adversarial defenses and a single-use server-chained Active Light protocol. The full color sequence is never disclosed up front; each next pulse is released only after the previous server state is consumed. Automatic approval remains disabled."
        tone="amber"
        meta={<><span className="inline-flex items-center gap-2 rounded-xl border border-cyan-300/12 bg-cyan-300/[0.025] px-3 py-2 text-xs text-cyan-200"><ShieldCheck className="size-3.5" /> Server-chained liveness</span><span className="inline-flex items-center gap-2 rounded-xl border border-violet-300/12 bg-violet-300/[0.025] px-3 py-2 text-xs text-violet-200"><ShieldCheck className="size-3.5" /> Adversarial CI</span><span className="inline-flex items-center gap-2 rounded-xl border border-amber-300/12 bg-amber-300/[0.025] px-3 py-2 text-xs text-amber-200"><TriangleAlert className="size-3.5" /> Review-only trust boundary</span></>}
      ...
      <section className="grid gap-5 xl:grid-cols-[1.15fr_.85fr]">
        <Card className="glass-panel"><CardHeader><CardTitle>Campaigns</CardTitle><CardDescription>Challenge policy and proof lifetime. New campaigns can be created directly from this console.</CardDescription></CardHeader><CardContent className="space-y-3">{campaigns.length === 0 ? <div className="rounded-2xl border border-dashed border-border p-6 text-sm text-slate-400">No Humanity campaigns exist yet. Create the first campaign above, then use it in the integrated liveness sandbox.</div> : campaigns.map((campaign) => <div key={campaign.id} className="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-medium text-white">{campaign.name}</p><p className="mt-1 text-xs text-slate-500">{campaign.slug}</p></div><Badge variant="outline" className={campaign.humanityGateEnabled ? "border-emerald-300/20 text-emerald-200" : "border-slate-300/20 text-slate-300"}>{campaign.humanityGateEnabled ? "Enabled" : "Disabled"}</Badge></div><div className="mt-4 grid grid-cols-2 gap-3 text-xs text-slate-400 sm:grid-cols-4"><span>Level <strong className="text-slate-200">{campaign.challengeLevel}</strong></span><span>Attempts <strong className="text-slate-200">{campaign.maxAttemptsPerWallet}</strong></span><span>Sessions <strong className="text-slate-200">{campaign._count.sessions}</strong></span><span>Proofs <strong className="text-slate-200">{campaign._count.verifications}</strong></span></div></div>)}</CardContent></Card>

        <Card className="glass-panel border-amber-300/15"><CardHeader><CardTitle className="flex items-center gap-2"><ShieldCheck className="size-5 text-amber-300" /> Current security boundary</CardTitle><CardDescription>What Tri-Proof Humanity Liveness does and does not claim.</CardDescription></CardHeader><CardContent className="space-y-3 text-sm leading-6 text-slate-300"><p><CheckCircle2 className="mr-2 inline size-4 text-emerald-300" />Motion sequence, nonce, expiry and attempt policy remain server-issued.</p><p><CheckCircle2 className="mr-2 inline size-4 text-emerald-300" />The full Active Light color sequence is concealed. Each pulse is disclosed only after the previous single-use server state is accepted.</p><p><CheckCircle2 className="mr-2 inline size-4 text-emerald-300" />Server state transitions use signed short-lived tokens plus database compare-and-swap on the Humanity session, so replayed or forked chain states are rejected.</p><p><CheckCircle2 className="mr-2 inline size-4 text-emerald-300" />Capture integrity evaluates cadence, media-time continuity, presented-frame order, loop signatures and face-motion/pixel-motion coupling.</p><p><CheckCircle2 className="mr-2 inline size-4 text-emerald-300" />Adversarial CI covers static photo, prerecorded loop, digital tint, frame injection, impossible timing and synthetic-media-like motion decoupling.</p><p><CheckCircle2 className="mr-2 inline size-4 text-emerald-300" />Raw video and device identifiers are not persisted. Short-lived chain state is carried by the client; the server persists normal session and abuse-control state.</p><p><TriangleAlert className="mr-2 inline size-4 text-amber-300" />Capture metadata remains forgeable under a fully compromised browser. Humanity Liveness therefore stays approval-ineligible until stronger hardware/PAD assurance is justified by evidence.</p></CardContent></Card>
      </section>

      <Card className="glass-panel"><CardHeader><CardTitle>Recent verifications</CardTitle><CardDescription>Newest persisted Humanity records, including wallet-signature state.</CardDescription></CardHeader><CardContent className="space-y-3">{recentVerifications.length === 0 ? <div className="rounded-2xl border border-dashed border-border p-6 text-sm text-slate-400">No Humanity verification has been submitted yet.</div> : recentVerifications.map((verification) => <div key={verification.id} className="grid gap-3 rounded-2xl border border-white/[0.07] bg-white/[0.02] p-4 md:grid-cols-[1fr_auto] md:items-center"><div><div className="flex flex-wrap items-center gap-2"><p className="font-medium text-white">{verification.campaign.name}</p><Badge variant="outline" className={decisionClass(verification.decision)}>{verification.decision}</Badge>{verification.signatureVerified ? <Badge variant="outline" className="border-cyan-300/20 text-cyan-200">Signed</Badge> : null}</div><p className="mt-2 break-all font-mono text-xs text-slate-500">{verification.walletAddress}</p></div><div className="text-left md:text-right"><p className="text-sm font-medium text-slate-200">Score {Math.round(verification.humanSessionScore)}</p><p className="mt-1 text-xs text-slate-500">{verification.createdAt.toISOString()}</p></div></div>)}</CardContent></Card>
    </div>
  )
}