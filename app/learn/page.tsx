...
const guardianSteps = [
  ...
  ["Review repeated activity", "When the same sender repeatedly posts high-risk items or repeats a target, Group Guardian displays sender behavior and can offer eligible administrators a one-hour mute action."],
] as const
...
export default function LearnPage() {
  return (
    <main className="premium-page min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <ProductSection id="group-guardian" eyebrow="Module 05 · community defense" title="Protect a Telegram community." description="Group Guardian is for teams that want automatic risk alerts, repeated-campaign detection, history, daily summaries, and moderation support inside a managed Telegram group." icon={UsersRound} steps={guardianSteps} actions={<><Link href="/dashboard/admin/telegram" className={`${buttonVariants()} glow-primary`}>Manage groups <ArrowRight data-icon="inline-end" /></Link><Link href="/pricing" className={buttonVariants({ variant: "outline" })}>View Community plan</Link></>} aside={<><strong className="text-white">Recommended setup:</strong> begin with a HIGH_RISK threshold. Add the bot as an admin only after connecting the correct group, and grant Restrict members only if the team wants automated mute controls.</>} />
